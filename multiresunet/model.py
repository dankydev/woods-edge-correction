...
class Conv2DBatchNorm(nn.Module):
    def __init__(self, input_features: int, num_of_filters: int, kernel_size: Tuple = (2, 2), stride: Tuple = (1, 1),
        ...
        self.conv = nn.Conv2d(in_channels=input_features, out_channels=num_of_filters, kernel_size=kernel_size,
...
class Multiresblock(nn.Module):
    def __init__(self, input_features: int, corresponding_unet_filters: int, alpha: float = 1.67) -> None:
        ...
        self.conv2d_bn_1x1 = Conv2DBatchNorm(input_features=input_features,
                                             num_of_filters=int(self.W * 0.167) + int(self.W * 0.333) + int(
                                                 self.W * 0.5),
                                             kernel_size=(1, 1), activation='None', padding=0)

        self.conv2d_bn_3x3 = Conv2DBatchNorm(input_features=input_features, num_of_filters=int(self.W * 0.167),
                                             kernel_size=(3, 3), activation='relu', padding=1)
        self.conv2d_bn_5x5 = Conv2DBatchNorm(input_features=int(self.W * 0.167), num_of_filters=int(self.W * 0.333),
                                             kernel_size=(3, 3), activation='relu', padding=1)
        self.conv2d_bn_7x7 = Conv2DBatchNorm(input_features=int(self.W * 0.333), num_of_filters=int(self.W * 0.5),
                                             kernel_size=(3, 3), activation='relu', padding=1)
        self.batch_norm = nn.BatchNorm2d(int(self.W * 0.5) + int(self.W * 0.167) + int(self.W * 0.333), affine=False)
...
class Respath(nn.Module):
    def __init__(self, input_features: int, filters: int, respath_length: int) -> None:
        ...
        self.conv2d_bn_1x1 = Conv2DBatchNorm(input_features=input_features, num_of_filters=self.filters,
                                             kernel_size=(1, 1), activation='None', padding=0)
        self.conv2d_bn_3x3 = Conv2DBatchNorm(input_features=input_features, num_of_filters=self.filters,
                                             kernel_size=(3, 3), activation='relu', padding=1)
        self.conv2d_bn_1x1_common = Conv2DBatchNorm(input_features=self.filters, num_of_filters=self.filters,
                                                    kernel_size=(1, 1), activation='None', padding=0)
        self.conv2d_bn_3x3_common = Conv2DBatchNorm(input_features=self.filters, num_of_filters=self.filters,
                                                    kernel_size=(3, 3), activation='relu', padding=1)
        self.batch_norm = nn.BatchNorm2d(filters, affine=False)
...
class MultiResUnetEncoderCell(nn.Module):
    def calculate_step_filter(self, filters, alpha):
        return int(filters * alpha * 0.5) + int(
            filters * alpha * 0.167) + int(
            filters * alpha * 0.333)

    def __init__(self, channels: int, alpha, encoder_length: int, encoder_index: int = 0, filters: int = 32):
        super().__init__()
        self.alpha = alpha
        self.current_filters = filters * (2 ** encoder_index)
        self.input_features = self.calculate_step_filter(self.current_filters // 2, self.alpha)  if encoder_index > 0 else channels
        self.respath_length = encoder_length - encoder_index
        self.multiresblock = Multiresblock(input_features=self.input_features,
                                           corresponding_unet_filters=self.current_filters)
        if encoder_index == 0:
            self.pool = nn.MaxPool2d(2, stride=2)
        else:
            self.pool = nn.MaxPool2d(2, 2)
        self.in_filters = self.calculate_step_filter(self.current_filters, self.alpha)
        self.respath = Respath(input_features=self.in_filters, filters=self.current_filters,
                               respath_length=self.respath_length)

    def forward(self, x):
        multiresblocked = self.multiresblock(x)
        pooled = self.pool(multiresblocked)
        for_decoder = self.respath(multiresblocked)

        return pooled, for_decoder
...
class MultiResUnet(nn.Module):
    def __init__(self, channels: int, filters: int = 32) -> None:
        ...
        """self.block1 = MultiResUnetEncoderCell(alpha=self.alpha, channels=channels, encoder_length=4, encoder_index=0,
                                              filters=self.filters)
        self.block2 = MultiResUnetEncoderCell(alpha=self.alpha, channels=channels, encoder_length=4, encoder_index=1,
                                              filters=self.filters)
        self.block3 = MultiResUnetEncoderCell(alpha=self.alpha, channels=channels, encoder_length=4, encoder_index=2,
                                              filters=self.filters)
        self.block4 = MultiResUnetEncoderCell(alpha=self.alpha, channels=channels, encoder_length=4, encoder_index=3,
                                              filters=self.filters)"""
        ...
        self.conv_final = Conv2DBatchNorm(input_features=self.in_filters9, num_of_filters=3, kernel_size=(1, 1),
                                          activation='None')
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """pooled1, for_decoder1 = self.block1(x)
        pooled2, for_decoder2 = self.block2(pooled1)
        pooled3, for_decoder3 = self.block3(pooled2)
        pooled4, for_decoder4 = self.block4(pooled3)"""
...
if __name__ == "__main__":
    net = MultiResUnet(channels=3, filters=16)
    inp = torch.ones(3, 128, 128)
    out = net(torch.tensor(np.expand_dims(inp, axis=0)))

    print(np.shape(inp))
    inp_to_show = np.moveaxis(np.moveaxis(inp.numpy(), -1, 0), 2, 1)
    out_to_show = np.moveaxis(np.moveaxis(out.detach()[0].numpy(), -1, 0), 2, 1)
    cv2.imshow("input", inp_to_show)
    cv2.imshow("output", out_to_show)
    cv2.waitKey()
