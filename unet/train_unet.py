if __name__ == "__main__":
    ...
    mse_w, vgg_high_w, ms_ssim_w, vgg_low_w, dists_w, dss_w, fsim_w = conf.get("mseWeight"), conf.get("vggHighWeight"), conf.get("msSsimWeight"), conf.get("vggLowWeight"), conf.get("distsWeight"), conf.get("dssWeight"), conf.get("fsimWeight")
    loss = WoodCorrectionLoss(mse_w=mse_w, vgg_high_w=vgg_high_w, ms_ssim_w=ms_ssim_w, vgg_low_w=vgg_low_w, dists_w=dists_w, dss_w=dss_w, device=device, fsim_w=fsim_w).to(device)
    ...
    model_name = f"{current_datetime}_{exp_name}_unet_maxshift{max_shift}_minshift{min_shift}_mse{mse_w}_vgglow{vgg_low_w}_msssim{ms_ssim_w}_vgghigh{vgg_high_w}_dists{dists_w}_dss{dss_w}_fsim{fsim_w}_lr{lr}_batch{batch_size}_epochs{epochs}_freq{log_step_frequency}_replaceWithStride{replace_maxpool_with_stride}.pth"
